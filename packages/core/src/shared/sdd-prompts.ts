...
export const PROVISION_LIBRARY_PROMPT = [
  ...
  "",
  "Follow this recipe exactly (consume the real library; never reimplement what it ships):",
  ...
  "   headless; mui/chakra/antd/mantine → installed-package) or ask.",
  "2. Idempotency scan FIRST: skip components already present in `component_dir`; never duplicate.",
  "3. cli-registry (shadcn): run the library's own CLI NON-INTERACTIVELY (use `library_install_cmd`",
  "   then `library_add_cmd <components>`, e.g. `npx shadcn@latest init --yes --defaults` then",
  "   `npx shadcn@latest add --yes <components>`) so the REAL component source files land in",
  "   `component_dir`. Do NOT reimplement a component the CLI can produce.",
  "4. installed-package (mui/chakra/antd/mantine): install the package (`library_install_cmd`), then",
  "   generate one thin token-mapped wrapper per default primitive that imports the real component",
  "   from `library_import_base` and maps `token_file` values onto it. Wrappers delegate; never",
  "   reimplement. Do NOT copy the package's source.",
  "4b. Astryx (installed-package) specifics — CONFIRMED from astryx.atmeta.com/docs: after install +",
  "   `npx @astryxdesign/cli init`, ENUMERATE via the Astryx CLI (NOT `.d.ts`, a Storybook, or an MCP):",
  "   `astryx component` lists all components, `astryx component <Name>` gives props+usage, `astryx docs",
  "   tokens` gives the token reference. Invoke it reliably with `node node_modules/@astryxdesign/cli/bin/",
  "   astryx.mjs`. Its tokens are CSS custom properties (--color-*/--spacing-*/--radius-*/--size-*), so point",
  "   `token_file` at the imported theme CSS (`@astryxdesign/theme-<name>/theme.css`) and personalize via the",
  "   css-vars path. Components import from per-category subpaths (`@astryxdesign/core/Button`). Custom-theme",
  "   authoring is limited to Astryx's `defineTheme` (see `astryx docs theme`) — VortSpec overrides tokens but",
  "   cannot redefine the theme abstractly; SAY SO to the user if they ask for bespoke theme changes.",
  "5. headless (radix/headlessui): install the package (`library_install_cmd`) and import primitives",
  "   from `library_import_base`. These ship NO token model — style with the project's own tokens.",
  "6. Run CLIs through the project's local toolchain (npm/npx) — never sudo, never global. If a CLI",
  ...
  "7. Do NOT modify unrelated files. When done, report what was added vs. skipped and point the user",
