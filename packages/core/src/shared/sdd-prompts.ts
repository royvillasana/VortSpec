...
export const PROVISION_LIBRARY_PROMPT = [
  ...
  "   properties (--color-*/--spacing-*/--radius-*/--size-*). The `token_file` MUST import BOTH stylesheets,",
  "   in this order: `@import '@astryxdesign/core/astryx.css';` then",
  "   `@import '@astryxdesign/theme-<name>/theme.css';`. The theme package carries colour/radius/shadow, but",
  "   the SPACING and size scales live in CORE's stylesheet — importing only the theme leaves the design",
  "   system with no spacing scale at all (every reader follows the token file's @import chain, so tokens",
  "   outside it genuinely do not exist). Personalize via the css-vars path; components import",
