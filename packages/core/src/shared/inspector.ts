...
/**
 * AI-ready per-component metadata (Plan B6): a curated brief the agent reads when it
 * composes or edits with a component, so it uses it as intended instead of inferring
 * from source. Generated by a gated Claude Code run into `.vortspec/metadata/<name>.json`
 * (VortSpec-owned — never touches the app's source), loaded on demand.
 */
...
/** Coverage of AI-ready metadata across the component roster (Plan B6). */
export const metadataStatusSchema = z.object({
  total: z.number(),
  withMetadata: z.number(),
  /** Roster component names still missing a metadata file. */
