...
export function buildEnterpriseFoundationPrompt(cfg: EnterpriseConnectConfig): string {
  ...
  return [
    ...
    "   path + named export + Storybook story id (and tier). NEVER author a competing component definition.",
    "",
    "3. TOKENS -- find the client's REAL token source and POINT `token_file` at it (reference, don't copy).",
    "   Identify where the COMPLETE token set actually lives -- across ALL FIVE categories: color, spacing,",
    "   radius, typography (font-family/size/weight/line-height), and shadow. Do NOT stop at colors/radius;",
    "   spacing and shadows are the most-often-missed. Look, best first: their token file (CSS variables /",
    "   SCSS); their theme config (tailwind.config.js -- the FULL resolved theme: defaults MERGED with",
    "   theme.extend, since Tailwind's spacing/boxShadow/fontSize live in the DEFAULT theme, which is why",
    "   parsing only extend drops them -- or a JS/TS theme object); the Storybook's FOUNDATIONS/token stories;",
    "   the Storybook preview root custom properties. Then UPDATE `token_file` in `.sdd-de/project.yaml` to the",
    "   real source file path (e.g. tailwind.config.js or their tokens file) -- the inspector reads it there, so",
    "   the palette + Generate code reference THEIR source, never a VortSpec copy. VERIFY all five categories",
    "   resolve from it. ONLY if the tokens exist solely at RUNTIME (Storybook root vars, no source file at all)",
    "   should you write a token file from the harvested values as a last resort; otherwise never author one.",
...
export function buildEnterpriseSnapshotPrompt(
  ...
  return [
    ...
    "",
    "TOKENS: make sure `token_file` in `.sdd-de/project.yaml` POINTS at the client's REAL token source (their",
    "token file / SCSS, or their tailwind.config.js / theme object) -- reference it, do NOT write a VortSpec",
    "copy. Confirm the COMPLETE set resolves from it across ALL FIVE categories -- color, spacing, radius,",
    "typography (font-family/size/weight/line-height), and shadow. Do NOT stop at colors/radius; spacing and",
    "shadows are the most-often-missed (Tailwind's spacing/boxShadow/fontSize live in the DEFAULT theme, so the",
    "source must be read as the FULL resolved theme, not just theme.extend). If a category is missing there, also",
    "read the Storybook's Foundations/token stories to fill it. ONLY if the tokens exist solely at RUNTIME (root",
    "vars, no source file) write a token file as a last resort. These are the client's tokens; reference, never invent.",
    "",
