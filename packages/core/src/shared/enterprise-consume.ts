...
export function buildEnterpriseFoundationPrompt(cfg: EnterpriseConnectConfig): string {
  ...
  return [
    ...
    "",
    "3. TOKENS -> the configured token_file (so the Design System palette + Generate code can read them):",
    "   extract the client's design tokens with RESOLVED values from whichever source they use, in this order:",
    "   the Storybook preview `:root` CSS custom properties; else their token file (CSS vars / SCSS); else their",
    "   Tailwind/theme config (tailwind.config theme.extend, or a JS/TS theme object). WRITE them to the",
    "   project's configured token_file (from project.yaml) as CSS custom properties -- `:root { --name: value; }`",
    "   -- one variable per token, resolved value, grouped by category (color/spacing/radius/typography/shadow).",
    "   This token_file is what the palette reads; without it the palette shows no tokens. Do NOT invent tokens.",
...
export function buildEnterpriseSnapshotPrompt(
  ...
  return [
    ...
    "",
    "TOKENS: extract the client's design tokens with RESOLVED values -- from the Storybook preview `:root`",
    "custom properties if present, else from their token file / Tailwind theme / theme object -- and WRITE",
    "them to the project's configured token_file as CSS custom properties (`:root { --name: value; }`), so the",
