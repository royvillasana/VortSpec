...
export function buildEnterpriseFoundationPrompt(cfg: EnterpriseConnectConfig): string {
  ...
  return [
    ...
    "",
    "3. TOKENS -> the configured token_file. Extract the COMPLETE token set with RESOLVED values across ALL",
    "   FIVE categories -- color, spacing, radius, typography (font-family/size/weight/line-height), and shadow.",
    "   Do NOT stop at colors/radius: spacing and shadows are the most-often-missed. Sources, best first:",
    "   a. The Storybook's FOUNDATIONS / token stories -- the authoritative catalog. Scan the story index for",
    "      titles like Foundations/Design Tokens/Colors/Spacing/Radius/Typography/Shadows/Elevation; RENDER each",
    "      (`iframe.html?id=<id>&viewMode=story|docs`) and read every token's name + its resolved value from the",
    "      swatches/rows it displays. These pages enumerate the FULL set -- use them as the source of truth.",
    "   b. The Storybook preview `:root` CSS custom properties (getComputedStyle on the root).",
    "   c. Their token file (CSS variables / SCSS) if connected.",
    "   d. Their theme config -- resolve the FULL Tailwind theme (defaults MERGED with theme.extend, e.g. via",
    "      `resolveConfig`), NOT just theme.extend: Tailwind's spacing / boxShadow / fontSize live in the DEFAULT",
    "      theme, so parsing only `extend` is exactly why spacing + shadows go missing. Or a JS/TS theme object.",
    "   WRITE every token to the project's configured token_file (from project.yaml) as CSS custom properties --",
    "   `:root { --name: value; }` -- one variable per token, resolved value, grouped by category. This file is",
    "   what the palette reads; without a complete file the palette shows partial or no tokens. Do NOT invent tokens.",
    "4. PROJECT the light layer (the ONLY thing you create): snapshot their Storybook renders into",
    "   framework-free light stand-ins for the Playground (see the snapshot step).",
    "",
...
export function buildEnterpriseSnapshotPrompt(
  ...
  return [
    ...
    "",
    "TOKENS: extract the COMPLETE token set with RESOLVED values across ALL FIVE categories -- color, spacing,",
    "radius, typography (font-family/size/weight/line-height), and shadow. Do NOT stop at colors/radius; spacing",
    "and shadows are the most-often-missed. Best source first: the Storybook's FOUNDATIONS / token stories",
    "(titles like Foundations/Design Tokens/Colors/Spacing/Radius/Typography/Shadows) -- render each and read the",
    "token names + resolved values it displays (the authoritative catalog). Else the preview `:root` custom",
    "properties; else their token file/SCSS; else the FULL Tailwind theme (defaults MERGED with theme.extend, not",
    "just `extend` -- spacing/boxShadow/fontSize live in Tailwind's DEFAULT theme). WRITE every token to the",
    "configured token_file as CSS custom properties (`:root { --name: value; }`), grouped by category, so the",
    "Design System palette and Generate code read them. These are the client's tokens; reference, never invent.",
    "",
