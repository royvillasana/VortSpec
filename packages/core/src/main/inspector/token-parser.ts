...
import { DEFAULT_CONTEXT, parseCssContexts, resolveInContext } from "@vortspec/core/css-tokens";
...
/**
 * The context-aware CSS parse (change: figma-native-token-model) now lives in `shared/css-tokens.ts`
 * and is re-exported here so every existing importer is untouched (OpenSpec change:
 * agentic-design-system, task 7.10). It moved because reading a stylesheet is no longer only the
 * Inspector's concern — it is one of the canonical artifact's INGEST paths — and an ingest builder
 * has to stay fs-free.
 */
export {
  DEFAULT_CONTEXT,
  parseCssContexts,
  resolveInContext,
  type CssContextParse,
} from "@vortspec/core/css-tokens";
