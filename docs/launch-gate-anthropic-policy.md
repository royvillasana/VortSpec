...
- **The IDE MCP integration stays within this posture.** The assistant's editor context/control (see `openspec/changes/assistant-ide-integration`) is delivered *not* via the interactive `--ide` bridge (that never connects under headless `-p` — verified) but via (a) prompt-injected file/selection context and (b) a **local** VortSpec stdio MCP server passed with `--mcp-config`. The MCP server binds a **unix socket on the user's machine only** (per-run 256-bit token, 0600 files) — no network listener, no keys, still non-bare. Workspace-changing tools (`open_folder`/`clone_repo`/`switch_project`) are **user-confirmed in the IDE**; a model cannot silently act.
