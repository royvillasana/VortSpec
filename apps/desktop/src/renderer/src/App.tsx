          ...
          <AssistantDock
            ...
            seedContext={
              projectView === "preview"
                ...
                : projectView === "runapp"
                  ? "Context: the user is on the Run app screen with the project's app running live at localhost. Vibe-engineer screens with them: compose new screens/features from the BUILT design-system components (import them from the component directory) and the design tokens — read DESIGN.md as the hand-off, follow the SDD-DE Screen Creation approach (a spec/plan before implementing), never hardcode hex/px, and write normal project files. The dev server hot-reloads, so changes appear live in the embedded app. REACHABILITY — MANDATORY: every page/screen you create MUST be reachable and appear in the app's site tree (the sidebar). Never leave a page reachable only by clicking through the app. Router app: create the page's route file AND add a link to it in the app's navigation. State-navigated app (no router): add a nav control that navigates to it AND register it in .vortspec/screen-preview.json (append { name, file } for the new screen, keeping existing entries) so it deep-links via ?screen=<Name>. A page not wired into routing/navigation is incomplete. MULTI-STATE SCREENS: when one component renders several meaningfully-distinct preview states (a page shown for different products, device/theme variants, loading/empty/loaded states), ASK the user before registering the harness whether they want EACH state as its own navigable site-tree screen or a SINGLE screen, then register .vortspec/screen-preview.json accordingly (one entry per state with distinct names + the same file, or a single entry). Default to a single entry when unsure; never silently fan a component out into many site-tree entries."
