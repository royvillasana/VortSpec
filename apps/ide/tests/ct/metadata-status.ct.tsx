...
test.fixme("shows coverage and offers a generate action for the gap", async ({ mount }) => {
  let gen = 0;
...
// QUARANTINED [REGISTRY] — see QUARANTINE.md
test.fixme("shows a running state and disables the button", async ({ mount }) => {
...
// QUARANTINED [REGISTRY] — see QUARANTINE.md
test.fixme("shows a complete state with no button when every component is covered", async ({ mount }) => {
...
// QUARANTINED [REGISTRY] — see QUARANTINE.md
test.fixme("renders nothing when there is no roster", async ({ mount }) => {
