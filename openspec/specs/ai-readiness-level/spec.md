# ai-readiness-level Specification

## Purpose
A computed maturity level for the design system, derived from the artifacts the other capabilities already produce, with the specific gap that raises it. A scoreboard the system cannot flatter, because every input is measured rather than asserted.
