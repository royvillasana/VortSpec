# preview-inspector-bridge Specification

## Purpose
The guest-page instrumentation behind the Run Canvas: an Electron webview guest preload that reads the already-rendered DOM with no cooperation from the user's project, streams a node tree, computed styles and geometry to the host, maps rendered elements to source components and design tokens, and applies ephemeral CSS overrides for instant visual feedback.
