# agent-query-protocols Specification

## Purpose
Generated rule documents that teach a run to NAVIGATE the index rather than explore the repository: what each artifact answers, how to trace relationships, and what never to re-read. Project-scoped, because a protocol that names a tier or a path the project does not have sends a run looking for it.
