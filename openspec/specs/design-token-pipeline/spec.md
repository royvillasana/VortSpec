# design-token-pipeline Specification

## Purpose
One canonical token artifact scanned once and emitted to every consumer format, so the design source, the code, and every generated surface agree on what a token is and what it resolves to.
