...
**D0. VortSpec-native, not a toolkit skill.** The lightweight mode is a VortSpec product capability and is implemented in VortSpec's tracked packages (`packages/core` pure transforms + `main/` orchestration, `packages/ui`/`apps/*` surfaces), not as prose skills/docs in the portable, gitignored `.sdd-de/` toolkit. Rationale: it's specific to VortSpec's Playground and must be version-controlled with the app; the SDD-DE toolkit stays general-purpose and framework-portable. The toolkit's artifacts (`components.json`, specs, `DESIGN.md`) are consumed as read-only inputs. *Alternative:* ship it as a toolkit skill (rejected — pollutes the portable toolkit with VortSpec-only behavior and leaves it untracked here).
...
**D4. Derive component stand-ins from Figma (via the read-only MCP), NOT from Storybook.** The one missing field (framework-free component structure) is filled by reading each component's Figma node — geometry, fills, text, layout — during extraction (the agent step that already uses the read-only Figma MCP; VortSpec never calls Figma directly) and converting it to a framework-free, token-referenced HTML stand-in. Figma is the design source of truth, so a Figma-derived stand-in is faithful *immediately* — no waiting for the framework build, no Storybook. The `harvest.ts` transform (node-tree → framework-free HTML) is source-agnostic and consumes the Figma node data. *Rejected — harvest the rendered DOM from Storybook:* it needs the framework component to already exist and Storybook running (breaks light-first, adds heavy infra), and it's redundant — the Figma node and the framework component derive from the *same* design, so re-capturing the framework's render gains nothing. *Also rejected — author stand-ins by hand* (net-new manual work).

**D5. One Figma read, light-HTML-FIRST, framework sequentially after.** Each component's Figma node is read **exactly once**; from that single read the **light HTML stand-in is emitted first** (so the live preview is available immediately), and the framework component is generated **later, sequentially**, reusing the same read. Both derive from the same Figma design over the same `extract-design-system` contract, so identity converges by construction and the visual is faithful from the start (no eventual-consistency gap). *Alternative:* two agents reading Figma independently (rejected — redundant reads + drift risk; the compile mapping breaks the moment the light `Button` ≠ the framework `Button`).
