...
- **Push target → VortSpec's own auto-created collection.** The push writes into a dedicated collection named `VortSpec` that VortSpec owns; if it doesn't exist, the writer creates it (`figma.variables.createVariableCollection` on the CLI path; `figma_create_variable_collection` on the MCP path) and writes the tokens there. This removes the "name a Tokens collection first" friction: the user never has to create or name a collection. The push stays confined to that one collection and never touches the user's other collections. (Superseded the earlier fix-it-on-absent-collection decision at the user's request.)
