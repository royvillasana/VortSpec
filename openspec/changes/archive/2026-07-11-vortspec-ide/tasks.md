...
- [x] 7.4 Update the website (`site/index.html`) to cover the two-app model, the terminal, and the IDE workflow: new "Two apps, one engine" section (`#apps`) contrasting the guided Cockpit and the VS Code–style IDE (Monaco + tree + Git diffs, integrated terminal, live preview, vibe-engineering, the SDD-DE pipeline panel) with an IDE window mock; hero + nav + download band updated (Cockpit + IDE ship in one release). HTML validated (block tags balanced); live screenshot deferred (Chrome extension not connected here).
