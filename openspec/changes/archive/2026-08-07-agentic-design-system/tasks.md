...
- [x] 9.1 DONE upstream — shipped in `@royvillasana/sdd-de@1.21.0` (PR royvillasana/SDD-DE#1). `enterprise` is now Branch G in `/setup` with its consume questions (package or repo, Storybook URL, token POINTER), and the CLI picker offers it too — it previously offered five sources while the skills documented five branches, so the new sources were unreachable from `npx sdd-de` as well
- [x] 9.2 DONE — skills half shipped in 1.21.0: Branch B is now `library | enterprise` and Branch A is `figma | claude-design` across enrich-brief, generate-artifacts, visual-verify and sync-tokens, each stating the consume rules (base component never recreated, customization is an overlay, and sync-tokens NEVER writes to a consume source's `token_file` — it lives inside a dependency). App half was already done: `App.tsx` now keys the Storybook/manifest hiding on `isConsumeSource`
