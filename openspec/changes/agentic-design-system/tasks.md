...
- [~] 2.10 PARTIAL — harness built (`prepareBenchmark`) and TOKEN COST measured on a real 55-component library: digest 1,045 → 1,538, +493 tokens, bounded by `MAX_RELATIONSHIPS` (recorded in §1.6). Accuracy, variance, false negatives and speed still need N independent agent trials graded against `answersFromGraph`'s key — the board used 11 — and grading an agent against a key derived from the same index would be circular for accuracy, so the harness prepares trials rather than faking them. ALSO FOUND: the benchmark needs an APPLICATION, not a component library — `packages/ui` resolves no entry page, so Q2–Q4 have no subject there; a valid target needs a resolvable entry page and at least two pages sharing a component
