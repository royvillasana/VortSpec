...
- [~] 2.10 PARTIAL — the benchmark now RUNS end to end on a real 55-component library, using a generated validation page as the entry page (group 2b): Q1 66, Q2 55, Q3 55 (an artifact of a synthetic all-atom roster — re-measure against real tiers), Q4 43 genuinely reused elsewhere. Token cost measured: digest 1,051 → 1,591, +540 tokens, bounded by `MAX_RELATIONSHIPS` (§1.6). REMAINING: accuracy, run-to-run variance, false negatives and speed need N independent agent trials graded against `answersFromGraph`'s key — the board used 11 — and grading an agent against a key derived from the same index would be circular for accuracy, so the harness prepares trials rather than faking them
