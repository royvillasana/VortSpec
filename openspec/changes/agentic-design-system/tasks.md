...
- [x] 4.8 Surface governance findings and both reports in the Inspector Issues view, filterable by kind
...
- [x] 6.1 Read `.sdd-de/docs/component-standards.md` and `framework-config.md` and record the current authoritative file set per framework/language/styling combination — the scaffold codifies these, it does not invent a new standard
- [x] 6.2 Add the scaffold in `packages/core`, driven by `.sdd-de/project.yaml`, writing the implementation, variants (only where the styling approach separates them), test file, and barrel export, plus index registration where the project uses one
- [x] 6.3 Omit inapplicable files rather than emitting empty placeholders; assert no zero-content file is ever written
...
- [ ] 9.1 BLOCKED — NOT THIS REPOSITORY'S FILE. `.sdd-de/` is gitignored: it is a working copy of the published package `@royvillasana/sdd-de@^1.19.0`, whose source lives elsewhere. Editing the prose here would be lost on the next toolkit update and would never reach a user. The change belongs in that repo: add `enterprise` to the design-source question and give it a consume branch that asks for the library/repo to consume, its Storybook/docs URL, and where its tokens live, recording `token_file` as a POINTER
- [~] 9.2 PARTIAL — the skill half is blocked with 9.1 (same repository boundary): retitle each branch "Branch B — Component Library Flow (`design_source: library | enterprise`)" and state the consume rules. The APP half is DONE, and it had the same defect in code this repo owns: `App.tsx` hid the Storybook and Design-manifest tabs on `designSource === "library"` while its own comment said "a consumed-library project has no VortSpec Storybook" — true of both consume sources — so an `enterprise` project created by the app's own enterprise flow still showed a Storybook tab that would never fill. Now keyed on `isConsumeSource`
- [x] 9.3 DONE — `shared/skill-parity.ts` + test, run against the INSTALLED toolkit. It found a second missing source the task list never mentioned: `claude-design` has no branch in any of the four skills either. Both gaps are allowlisted WITH their reasons, and the check fails both when a new gap appears and when an allowlisted one is fixed — a known-failure list that is never pruned becomes a permanent one
