...
- [x] 2.6 Implement SDD-DE toolkit detection, install, and update, reporting the installed version (`workspace-toolkit`). → `toolkit-manager.ts` wired to the **real toolkit** (`@royvillasana/sdd-de`): detects `.sdd-de/project.yaml` + `.sdd-de/ai-specs/skills/`; commands `npx @royvillasana/sdd-de` / ` update`. **Verified** (absent→present detection). The CLI init is **interactive** (@clack prompts) so in-app install runs in the PTY (lands in D5/7.1); until then it points the user to run the command in a terminal, or a non-interactive `VORTSPEC_TOOLKIT_INSTALL_CMD` override. No installed-version marker exists in the toolkit, so version shows "installed".
